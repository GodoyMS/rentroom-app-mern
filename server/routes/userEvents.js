...
router.get("/profile", (req, res) => {
  res.header("Access-Control-Allow-Origin", FRONTURL); //    res.header('Access-Control-Allow-Origin', 'https://rentroom-app-mern.vercel.app');
  res.header(
  ...
  if (token) {
    jwt.verify(token, jwtSecret, options, async (err, decodedUserData) => {
      if (err) throw err;
      const { name, email, shortDescription, _id } = await User.findById(
        decodedUserData.id
      );
...
router.put("/update-profile", async (req, res) => {
  ...
  jwt.verify(token, jwtSecret, options, async (err, userData) => {
    if (err) throw err;
...
router.get("/user-rentplaces", (req, res) => {
  res.setHeader("Access-Control-Allow-Origin", FRONTURL); //    res.setHeader("Access-Control-Allow-Origin", "https://rentroom-app-mern.vercel.app");
...
router.post("/account/rentplaces/post", (req, res) => {
  ...
  jwt.verify(token, jwtSecret, options, async (err, decodedUserData) => {
    ...
    const placeDoc = await RentPlace.create({
      owner: decodedUserData.id,
      ownerName: decodedUserData.name,
      country,
      region,
      title,
      address,
...
router.post("/account/postfavorite", async (req, res) => {
  ...
  const { rentPlace, name, monthlyPrice } = req.body;
  const rentPlaceExists = await Favorites.findOne({
    rentPlace: rentPlace,
    user: userData.id,
  });
